    ...
    db.prepare(
      `INSERT INTO ctx_usage
        (session_key, agent_id, total_tokens, last_model, last_model_ctx_max,
         original_model, last_warned_threshold, peak_percent, last_updated_at, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
export function getAgentCtxProfile(
  db: Database,
  agentId: string,
): { sessions: number; avgPeak: number; maxPeak: number } {
  try {
    const row = db
      .prepare(
        `SELECT COUNT(*) AS n, AVG(peak_percent) AS avg_peak, MAX(peak_percent) AS max_peak
         FROM ctx_usage WHERE agent_id = ?`,
      )
      .get(agentId) as { n: number; avg_peak: number | null; max_peak: number | null };
    return {
      sessions: row.n ?? 0,
    ...
    return { sessions: 0, avgPeak: 0, maxPeak: 0 };
  }
}
