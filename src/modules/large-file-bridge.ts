...
如果你或用户需要传输大文件（>100MB），请告知用户通过以下链接上传：

📎 上传页面: ${url}

上传后将获得分享链接，可直接在聊天中发送。也可使用 enhance_share_file 工具将本地文件转为分享链接。
(由 enhance large-file-bridge 生成；关闭: config.largeFileBridge.enabled = false)`;
}
