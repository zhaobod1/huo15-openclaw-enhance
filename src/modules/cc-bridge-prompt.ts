  ...
  try {
    api.registerMemoryPromptSupplement(() => {
      ...
      return [
        ...
        ``,
        `**严格区分：/lanhuo 跟 /upload 是不同 URL（v6.7.6）**：`,
        `📊 \`/lanhuo\` = 蓝火任务 dashboard（看 cc-media-task 进度 / 历史，不是上传文件页面）`,
        `📎 \`/upload\` = 大文件上传专用页面（企微 >100MB 文件走这里上传给 AI；最短最易记 URL）`,
        `    备用等价 URL：/lanhuo/upload / /plugins/enhance/upload（都返同一份上传页）`,
        `**不要混用**：用户问任务状态 → 给 /lanhuo；用户要上传大文件 → 给 /upload。`,
        `如果你的回复里出现『上传』/『传文件』/『100M无法下载』关键词 → 100% 要用 /upload 而**不是** /lanhuo。`,
        ``,
