...
- **OpenClaw plugin id 改 `enhance-huo15`**（ClawHub code-plugin 发布必需）：ClawHub 上 code-plugin 的插件 id `enhance` 被老包 `@huo15/openclaw-enhance`（owner huo15，非本仓可操作账号）占用，新 slug `huo15-huo15-openclaw-enhance` 无法以 `enhance` id 注册 code-plugin。经决策改为 `enhance-huo15`（保留 enhance 语义 + huo15 标识）。本地数据目录（`~/.openclaw/extensions/enhance`、`~/.openclaw/share`、`~/.openclaw/upload`）与 URL 前缀（`/plugins/enhance*`）不变；老用户 openclaw.json 的 `plugins.entries.enhance.*` 配置需迁移为 `plugins.entries.enhance-huo15.*`。见 README 一键安装段警告。
