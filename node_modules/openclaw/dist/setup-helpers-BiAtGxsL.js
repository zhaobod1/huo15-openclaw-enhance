...
function resolveSingleAccountKeysToMove(params) {
	...
	const namedAccountPromotionKeys = getChannelSetupPromotionSurface(params.channelKey)?.namedAccountPromotionKeys;
	return Object.entries(params.channel).filter(([key, value]) => {
		...
		if (hasNamedAccounts && namedAccountPromotionKeys && namedAccountPromotionKeys.length > 0 && !namedAccountPromotionKeys.includes(key)) return false;
		return true;
	}).map(([key]) => key);
}
function resolveSingleAccountPromotionTarget(params) {
	...
	if (typeof resolved === "string" && resolved.trim()) return resolveExistingAccountId(resolved);
