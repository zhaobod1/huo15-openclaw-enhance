...
<Response>
  <Connect>
		...
		bridge = this.realtimeProvider.createBridge({
			...
			onClose: (reason) => {
				...
				this.provider.hangupCall({
					...
					console.warn(`[voice-call] Failed to hang up realtime call ${callSid}: ${error instanceof Error ? error.message : String(error)}`);
				});
			}
		});
	...
	async executeToolCall(bridge, callId, bridgeCallId, name, args) {
		...
		const result = !handler ? { error: `Tool "${name}" not available` } : await handler(args, callId).catch((error) => ({ error: error instanceof Error ? error.message : String(error) }));
