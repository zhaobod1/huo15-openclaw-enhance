import { f as isPathInside$1, i as openBoundaryFileSync, n as matchBoundaryFileOpenFailure } from "./boundary-file-read-CdxVvait.js";
import { f as resolveConfigDir, l as isRecord, m as resolveUserPath } from "./utils-ms6h9yny.js";
import { a as resolvePackageExtensionEntries, i as loadPluginManifest, r as getPackageManifestMetadata, t as DEFAULT_PLUGIN_ENTRY_CANDIDATES } from "./manifest-BLZdOZfM.js";
...
function resolveBundledDirFromPackageRoot(packageRoot, preferSourceCheckout) {
	...
	if (isSourceCheckoutRoot(packageRoot) && fs.existsSync(sourceExtensionsDir)) return sourceExtensionsDir;
}
...
function listBundledChatChannelEntries() {
	return listChannelCatalogEntries({ origin: "bundled" }).flatMap(({ channel }) => {
		...
		return [{
			id,
			aliases: (channel.aliases ?? []).map((alias) => normalizeChannelKey(alias)).filter((alias) => Boolean(alias)),
...
//#endregion
export { loadBundleManifest as _, clearPluginDiscoveryCache as a, resolvePluginSourceRoots as c, safeRealpathSync as d, safeStatSync as f, detectBundleManifestFormat as g, CURSOR_BUNDLE_MANIFEST_RELATIVE_PATH as h, listChannelCatalogEntries as i, resolveBundledPluginsDir as l, CODEX_BUNDLE_MANIFEST_RELATIVE_PATH as m, CHAT_CHANNEL_ORDER as n, discoverOpenClawPlugins as o, CLAUDE_BUNDLE_MANIFEST_RELATIVE_PATH as p, normalizeChatChannelId as r, resolvePluginCacheInputs as s, CHANNEL_IDS as t, isPathInside as u, mergeBundlePathLists as v, normalizeBundlePathList as y };
