...
function resolveGeminiCliSelectedAuthType() {
	...
	if (settings) {
		...
		const selectedAuthType = readString(auth?.selectedType) ?? readString(auth?.enforcedType) ?? readString(settings.selectedAuthType) ?? readString(settings.enforcedAuthType);
